...
int lcd_test()
{
    ...
    {
        /* red */
        // 这个循环遍历LCD缓冲区的一半，因为缓冲区中的每个像素点由两个字节表示，一个表示高8位颜色信息，另一个表示低8位颜色信息。
        for (int i = 0; i < LCD_BUF_SIZE / 2; i++)
        {
            // 将高8位颜色信息设置为0，对于红色来说，意味着绿蓝两色的亮度为0。
            lcd->lcd_info.framebuffer[2 * i] = 0x00;
            // 将低8位颜色信息设置为0xF8，代表红色的亮度。
            lcd->lcd_info.framebuffer[2 * i + 1] = 0xF8;
        }
        // 调用设备的control方法，更新LCD的显示区域。RTGRAPHIC_CTRL_RECT_UPDATE是一个预定义的控制命令，用于通知LCD更新其显示的内容。
        lcd->parent.control(&lcd->parent, RTGRAPHIC_CTRL_RECT_UPDATE, RT_NULL);
        rt_thread_mdelay(1000); // 延时一秒
        /* green */
        for (int i = 0; i < LCD_BUF_SIZE / 2; i++)
        {
